...
class Scheduler:
    ...
    def get_balance_score(self):
        ...
        return sum(v ** 2 for v in slot_counts.values())
        ...
        for d in DAYS:
            for s in SLOTS:
                ...
                if class_load < self.class_limits[assignment['sinif']]:
                    potential_slots.append((d, s, class_load, global_load))

        random.shuffle(potential_slots)
        potential_slots.sort(key=lambda x: (x[2], x[3]))

        for day, slot, _, _ in potential_slots:
    ...
    def report_conflicts(self):
        ...
        if not found: print("Hiçbir sınıf çakışması bulunmuyor.")
        print("=" * 70 + "\n")
...
if __name__ == "__main__":
    try:
        ...
        print(f"Planlama başlatıldı. (Deneme Sayısı: {MAX_TRIALS})")
        ...
        for trial in range(1, MAX_TRIALS + 1):
            ...
            if scheduler.backtrack():
                ...
                print(f"Deneme {trial}/{MAX_TRIALS} başarılı. (Puan: {score})")
            else:
